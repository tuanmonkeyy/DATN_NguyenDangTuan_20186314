...
// Get end DPU time
	...
	dpu_time += (float) chrono::duration_cast<chrono::milliseconds>(t_dpu_2 - t_dpu_1).count();
		...
		if(results.bboxes.empty()) {
			...
			// Write indication
			rectangle(image, Point(textX - PIXEL_OFFSET, textY + PIXEL_OFFSET), Point(textX + textsize.width + PIXEL_OFFSET, textY - textsize.height - PIXEL_OFFSET), CV_RGB(255, 255, 255), FILLED, LINE_AA);
		...
		for (auto& box : results.bboxes) {
			...
			// Draw box
			rectangle(image, Point(xmin, ymin), Point(xmax, ymax), getColor(label, nb_classes), RECT_THICKNESS, FILLED, LINE_AA);
			// Write label
